...
SELECT @CustomerName = ISNULL(@CustomerName +', ', '') + '[' + CustName + ']'
FROM (SELECT DISTINCT (CustomerName) as CustName FROM Sales.Customers) AS q1
...
PIVOT (count(CustomerName) FOR CustomerName IN (' + @CustomerName + ')) AS pivotTable'
